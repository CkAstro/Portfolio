...
export const Canvas3d: React.FC<Canvas3dProps> = ({ renderFrame, drawTrigger, options }) => {
   ...
   // const timeArrayRef = useRef(Array.from({ length: 11 }, (_, i) => Date.now()));
   ...
   //       timeArrayRef.current.pop();
   //       timeArrayRef.current.unshift(Date.now());
   //       frameRateRef.current = 100000 / (timeArrayRef.current[0] - timeArrayRef.current[10]);
   ...
   const animateCanvas = useCallback(() => {
      ...
      if (
         optionsRef.current?.renderAtRefreshRate === true ||
         drawTrigger.current !== triggerComparison.current
      ) {
         // updateFrameRate();
